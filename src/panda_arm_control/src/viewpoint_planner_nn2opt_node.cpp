...
class ViewpointPlannerNn2optNode : public rclcpp::Node
{
public:
	explicit ViewpointPlannerNn2optNode(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
		: Node(
			  "viewpoint_planner_nn2opt",
...
private:
	...
	void runPipeline()
	{
		...
		PrintTourCostBreakdown(
			this->reachable_candidates, this->selected, travel_cost_matrix, this->params.joint_distance_weight);

		double achieved_visibility = ComputeAreaVisibility(simplified_mesh, this->selected);
		RCLCPP_INFO(
			this->get_logger(), "NN+2-opt (real travel_cost_matrix) reordered %zu selected views, %.2f%% area visibility",
...
int main(int argc, char* argv[])
{
	...
	auto node = std::make_shared<ViewpointPlannerNn2optNode>();
